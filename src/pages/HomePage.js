...
const HomePage = () => {
    ...
    const fetchPopularSeries = ()=>{
        fetch("https://api.themoviedb.org/3/tv/popular?api_key=e8d8f61f887dbb73c841557fc8ce3e6b&language=en-US&page=1&with_original_language=en")
        .then(response =>response.json())
        .then(data => setPopularShows(data.results))
        }

    const fetchNetflixSeries = ()=>{
        fetch("https://api.themoviedb.org/3/discover/tv?api_key=e8d8f61f887dbb73c841557fc8ce3e6b&language=en-US&sort_by=popularity.desc&page=1&timezone=America%2FNew_York&include_null_first_air_dates=false&with_watch_providers=8&watch_region=GB&with_watch_monetization_types=flatrate&with_status=0&with_type=0&with_original_language=en")
        .then(response => response.json())
